...
export function AuthPage(state) {
    const isLogin = state.loginMode === 'login';
    const isAdminAuth = state.authTab === 'admin';
    // Backticks added correctly below
    return `
    <div class="min-h-screen flex items-center justify-center p-4 bg-slate-900">
        <div class="w-full max-w-md glass-panel rounded-2xl shadow-2xl overflow-hidden relative z-10 fade-in border border-slate-700">
            <div class="p-8 text-center border-b border-slate-700/50">
                <h1 class="text-2xl font-bold text-white tracking-tight mb-1">StegoSecure</h1>
            </div>
            ...
            <div class="p-8 pt-6">
                ...
                <div class="space-y-4">
                    <input id="u-in" type="text" class="w-full px-4 py-2.5 bg-slate-950/50 border border-slate-700 rounded-lg text-sm text-white" placeholder="Username" />
                    <input id="p-in" type="password" class="w-full px-4 py-2.5 bg-slate-950/50 border border-slate-700 rounded-lg text-sm text-white" placeholder="Password" />
                    <button id="auth-action-btn" class="w-full mt-2 bg-blue-600 hover:bg-blue-500 text-white py-3 rounded-lg font-bold text-sm">
                        ${isAdminAuth ? 'Access Console' : (isLogin ? 'Establish Link' : 'Register Identity')}
                    </button>
                </div>

                ${state.error ? `<div class="mt-4 p-3 bg-red-500/10 text-red-400 text-xs rounded-lg">${state.error}</div>` : ''}
                ${state.success ? `<div class="mt-4 p-3 bg-green-500/10 text-green-400 text-xs rounded-lg">${state.success}</div>` : ''}
                ${isAdminAuth ? `<div class="mt-6 text-center text-[10px] text-slate-600 font-mono">DEFAULT: admin / admin123</div>` : ''}
            </div>
        </div>
    </div>`;
...
export function DashboardPage(currentUserData, activeTab) {
    ...
    <div class="flex h-screen bg-slate-950 overflow-hidden">
        <aside class="w-64 bg-slate-900 border-r border-slate-800 flex flex-col hidden md:flex">
            <div class="p-6 border-b border-slate-800"><h1 class="text-white font-bold">StegoSecure</h1></div>
            <nav class="flex-1 p-4 space-y-2">
                <button onclick="window.setActiveTab('chat')" class="nav-btn ${activeTab==='chat'?'active':''} w-full text-left p-2 text-slate-400 hover:text-white">Chat</button>
                <button onclick="window.setActiveTab('downloads')" class="nav-btn ${activeTab==='downloads'?'active':''} w-full text-left p-2 text-slate-400 hover:text-white">Gallery</button>
                <button onclick="window.setActiveTab('classifier')" class="nav-btn ${activeTab==='classifier'?'active':''} w-full text-left p-2 text-slate-400 hover:text-white">AI Scan</button>
                <button onclick="window.setActiveTab('decode')" class="nav-btn ${activeTab==='decode'?'active':''} w-full text-left p-2 text-slate-400 hover:text-white">Decrypt</button>
                ${currentUserData.role === 'admin' ? `<button onclick="window.setActiveTab('admin')" class="nav-btn ${activeTab==='admin'?'active':''} w-full text-left p-2 text-slate-400 hover:text-white">Admin</button>` : ''}
                ${currentUserData.role === 'admin' ? `<button onclick="window.setActiveTab('encode')" class="nav-btn ${activeTab==='encode'?'active':''} w-full text-left p-2 text-slate-400 hover:text-white">Encode</button>` : ''}
            </nav>
            <div class="p-4 border-t border-slate-800">
                <div class="text-white text-sm mb-2">${currentUserData.username}</div>
                <button onclick="window.handleLogout()" class="text-red-400 text-xs">Logout</button>
            </div>
        </aside>
        <main class="flex-1 bg-slate-950" id="tab-content"></main>
    </div>`;
}

export function renderChatTemplate(selectedChatUser, messages, currentUserData) {
    if (!selectedChatUser) return `<div class="p-10 text-center text-slate-500">Select a user</div>`;
    
    const msgs = messages.map(m => `
        <div class="mb-2 ${m.sender === currentUserData.username ? 'text-right' : 'text-left'}">
            <div class="inline-block p-2 rounded bg-slate-800 text-white text-sm max-w-xs break-words">
                ${m.type === 'image' ? `<img src="${m.imageUrl}" class="max-w-full rounded"/>` : escapeHtml(m.text)}
            </div>
        </div>`).join('');

    return `
    <div class="flex flex-col h-full">
        <div class="p-4 border-b border-slate-800 text-white font-bold">${selectedChatUser.username}</div>
        <div id="chat-messages" class="flex-1 overflow-y-auto p-4">${msgs}</div>
        <div class="p-4 border-t border-slate-800 flex gap-2">
            <input type="file" id="img-input" class="hidden">
            <label for="img-input" class="p-2 text-slate-400 cursor-pointer">📎</label>
            <form id="chat-form" class="flex-1 flex gap-2">
                <input id="text-input" class="flex-1 bg-slate-800 border border-slate-700 rounded p-2 text-white" autocomplete="off" placeholder="Message..."/>
